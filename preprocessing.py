...
def create_transaction_df(df):
    ...
    df_transac = df.groupby(['client_id', 'date_order']).agg(
...
def count_orders(df_transac, df_features):
    df_client = df_transac.groupby('client_id').agg(
        nb_orders=('date_order', 'count')
    )

    df_features = pd.merge(df_features, df_client,
                           on=['client_id'],
                           how='left'
                           )
...
def avg_frequency_orders(df_transac, df_features):
    ...
    df_features['avg_freq_orders'] = np.where(df_features['time_delta'] == 0,
                                              0,
...
def average_basket(df_features):
    df_features['avg_basket'] = df_features['total_sales_net'] / df_features['nb_orders']

    return df_features

def pct_order_channel(df_transac, df_features):
    df_client = df_transac.groupby('client_id')['order_channel'].value_counts(normalize=True).unstack(fill_value=0)
    df_client.columns = ['pct_' + col.lower().replace(' ', '_') for col in df_client.columns]  # Rename columns
    df_client.reset_index(inplace=True)  # Reset index to make client_id a regular column
...
def time_since_last_order(df_transac, df_features, end_date):
    df_client = df_transac.groupby('client_id').agg(
        time_since_last_order=('date_order',
                        lambda x: (end_date - x.max()).days)
    )
